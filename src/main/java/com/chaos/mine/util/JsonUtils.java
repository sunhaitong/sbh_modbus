...
public class JsonUtils {
    ...
    public static Map<Integer, DeviceInfo> getDevIdList(String filePath) {
        ...
        ExcelReader reader = ExcelUtil.getReader(classPathResource.getStream());
     ...
     * @return ArrayList
     */
    public static Map<Integer, ModbusTcpData> gettBoxDevIdList(String filePath) {
        Map<Integer, ModbusTcpData> deviceInfoMap = new HashMap<>();
        // String src = "/usr/local/gateway/data_etl/" + filePath;
            /*    String src = "/data1/gateway/data_etl" + filePath;
            File file = new File(src);*/
        // 获取jar目录下的配置文件
        ...
        ExcelReader reader = ExcelUtil.getReader(classPathResource.getStream());
        int sheetCount = reader.getSheetCount();
        List<ModbusTcpData> allRecords = new ArrayList<>(sheetCount);
        for (int i = 0; i < sheetCount; i++) {
            reader.setSheet(i);
            List<ModbusTcpData> readAll = reader.readAll(ModbusTcpData.class);
            allRecords.addAll(readAll);
        }
        for (ModbusTcpData deviceInfo : allRecords) {
            if (!deviceInfoMap.containsKey(deviceInfo.getAddress())) {
                deviceInfoMap.put(deviceInfo.getAddress(), deviceInfo);
            }
        }
        return deviceInfoMap;
    }
